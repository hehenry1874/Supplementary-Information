...
SYSTEM_PROMPT = """
You are an expert port sustainability analyst. Your task is to perform a SECOND-PASS screening to extract structured readiness evidence from the provided document.
...
RULES:
1. ONLY assign stage > 0 if there is ANY evidence (even minor, indirect, or generic statements) in the text.
2. Be generous in identifying Stage 1 (Vision/commitment) or Stage 2 (Planning) evidence from news snippets or short texts. Capture generic mentions under Governance & Investment (e.g., policy_support_stage or strategy_roadmap_stage) if they loosely fit.
3. If the text does not mention a variable at all, assign stage 0.
4. Do not inflate stages to 3 or 4 without clear proof of construction/operation, but do capture 1 and 2 broadly to ensure no omissions.
"""
...
def main():
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="FILE",
        help="Drop these archived_file_path rows from wide partials, then fill pass2 for gaps.",
    )
    parser.add_argument(
        "--reprocess-from-file",
        type=Path,
        default=None,
        help="Text file with one basename per line.",
    )
    ...
    base_out_csv = SCRIPT_DIR / "evidence_wide.csv"
    out_csv = SCRIPT_DIR / "evidence_wide_pass2.csv"
    out_notes = SCRIPT_DIR / "evidence_extraction_notes_wide_pass2.md"
    ...
    # Determine processed files from the BASE output
    processed_files = set()
    if base_out_csv.exists():
        with open(base_out_csv, "r", encoding="utf-8") as f:
    ...
    # ALSO track files already processed in pass 2
    if out_csv.exists():
